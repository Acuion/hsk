...
def profile():
    ...
    if member:
        if member["player"]:
            userinfo = pgInstance().one("SELECT dep,name,death_word,secret_word,victims_data,score,killed_count,killed_list,anon_id,achievements,alive FROM hsspies_game WHERE vk_id={0}".format(repr(member["id"])), back_as=dict)
            ...
            if request.form.get("death_word") and victimId and 0 <= int(victimId) <= 1:
                ...
                if recap["success"] and request.form.get("death_word").lower().strip() == toBeKilled["death_word"]:
                    ...
                    victims = [pgInsiance().one("SELECT vk_id, name, dep FROM hsspies_game WHERE vk_id='{0}'".format(toBeKilled["victims_data"][0]["vk_id"])),
                        postgresdb.one("SELECT vk_id, name, dep FROM hsspies_game WHERE vk_id='{0}'".format(toBeKilled["victims_data"][1]["vk_id"]))]
                    killers = pgInstance().all("SELECT vk_id, victims_data FROM hsspies_game WHERE (victims_data->0->'vk_id' ? '{0}') OR (victims_data->1->'vk_id' ? '{0}')".format(toBeKilled["vk_id"]), back_as=dict)
                    #TODO: check it
                    for x in range(0, 2): #TODO: new victim system
                        if killers[x]["victims_data"][0]["vk_id"] == toBeKilled["vk_id"]:          
                            killers[x]["victims_data"][0] = victims[x]
                        else:
                            killers[x]["victims_data"][1] = victims[x]
                        pgInsitance().run("UPDATE hsspies_game SET victims_data='{0}' WHERE vk_id='{1}'".format(json.dumps(killers[x]["victims_data"]), killers[x]["vk_id"]))
